...
auth_handler = OAuth2Handler(
    client_id="car_rental_client_id", client_secret="car_rental_client_secret"
)
car_rental_api = APIClient(
    base_url="https://car-rental-enterprise-api.com", auth_handler=auth_handler
)


def search_cars(location: str, pickup_date: str, return_date: str):
    return car_rental_api.get(
        "/cars/search",
        params={
            "location": location,
...
def reserve_car(car_id: str, rental_details: dict):
    return car_rental_api.post(
        "/cars/reserve", data={"car_id": car_id, "rental": rental_details}
    )
